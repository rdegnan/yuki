...
let string_of_node write__a ?(len = 1024) x =
  let ob = Bi_outbuf.create len in
  write_node write__a ob x;
  Bi_outbuf.contents ob
let read_node read__a = (
  fun p lb ->
...
let node_of_string read__a s =
  read_node read__a (Yojson.Safe.init_lexer ()) (Lexing.from_string s)
...
let string_of_fg write__a ?(len = 1024) x =
  let ob = Bi_outbuf.create len in
  write_fg write__a ob x;
  Bi_outbuf.contents ob
let read_fg read__a = (
  fun p lb ->
    Yojson.Safe.read_space p lb;
...
let fg_of_string read__a s =
  read_fg read__a (Yojson.Safe.init_lexer ()) (Lexing.from_string s)
